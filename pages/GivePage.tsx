...
const PageHeader = () => (
    <div className="relative h-56 sm:h-64 bg-sg-purple flex items-center justify-center text-white text-center p-4">
        ...
        <div className="relative z-10">
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-serif font-bold">Give Generously</h1>
            <p className="text-base sm:text-lg lg:text-xl mt-2">Partner with us in advancing God's Kingdom.</p>
        </div>
    </div>
);

const DonationOption: React.FC<{ title: string; icon: React.ReactNode; children: React.ReactNode }> = ({ title, icon, children }) => (
    <div className="bg-white rounded-lg shadow-lg p-8 text-center transform hover:-translate-y-2 transition-transform duration-300 flex flex-col">
        <div className="flex justify-center items-center mb-4 text-sg-gold">{icon}</div>
        <h3 className="text-xl sm:text-2xl font-serif font-bold text-sg-purple mb-4">{title}</h3>
        <div className="text-gray-600 space-y-2 flex-grow flex flex-col justify-center">{children}</div>
    </div>
);
            ...
            <section className="py-16 sm:py-20">
                <div className="container mx-auto px-6">
                    <div className="text-center max-w-3xl mx-auto mb-16">
                        <h2 className="text-2xl sm:text-3xl lg:text-4xl font-serif font-bold text-sg-purple mb-4">Your Giving Makes a Difference</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        <DonationOption 
                            title="Mobile Money" 
                            icon={<svg className="w-12 h-12 sm:w-16 sm:h-16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>}
                        >
